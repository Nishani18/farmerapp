const en = {
  //Login Screen
  welcome: "Krishi Mithra",
  login: "Login",
  loginBtn: "Submit",
  loginInputPlaceholder: "Email",
  passwordInputPlaceholder: "Password",
  signUpBtn: "Sign Up",
  or: "or",

  //Register Screen
  register: "Register",
  email: "Email",
  fullname: "Full Name",
  confirmpassword: "Confirm Password",
  message1: "Minimum password length must be of 8 characters!",
  message2: "Password's don't match!",
  RegisterAlert: "Account Created",
  RegisterError: "Error creating your account!",

  //Home Screen
  hi: "Hi",
  totalCategories: "Total Categories",
  reminder: "Reminder",

  //Category Chart
  homeCategoryChart1: "No Categories yet!",
  homeCategoryChart2:
    "  Set up a Category to help you navigate with your expenses",

  //Reminder screen
  reminderMessage1: "No reminders set!",
  reminderMessage2: "Add a reminder to remind you for an expense later",
  reminderMessage3: "Add a reminder",
  Upcomingreminder: "Upcoming Reminders",
  reminderMessage4: "Long press to delete Reminders",
  reminderTitle: "Add Reminder",
  reminderDate: "Date",
  reminderTime: "Time",
  reminderInput: "Insert Note",
  reminderSubmit: "Submit",
  reminderRemaining: " days remaining",
  ReminderSucess: "Reminder set successfully",

  reminderDeleteSucess: "Reminder deleted successfully",
  reminderAlertDeleteTitle: "Delete Reminder",
  reminderAlertDeleteMessage: "Are you sure you want to delete this reminder?",

  //Category Screen
  categorytitle: "Categories",
  categoryMessage1: "No Categories yet!",
  categoryMessage2:
    "Click on the plus to add a new Category and make budgeting easier.",
  categorytoggle: "Add Category",
  caetgoryInput: "Category",
  categorysubmit: "Submit",
  cateoryAlertHead: "Delete Category",
  categoryAlertPara: "Are you sure you want to delete this category?",
  categoryDelete: "DELETE",
  categoryCancel: "CANCEL",
  categoryAddAlertMessages: "Category added successfully",
  categoryAddAlertErrorMessages: "Failed to add category",
  categoryDeleteAlertMessages: "Category deleted successfully",
  categoryDeleteAlertErrorMessages: "Failed to delete category",

  //Sub Category Screen
  subcategorytitle: "lists",
  subcategorymessage1: "No Sub Categories yet!",
  subcategorymessage2:
    " Set up a new Sub Category to help you keep track of your expenses",
  subcategorytoggle: "Add your list",
  subcategorytoggletitle: "Name your list",
  subcategoryestimate: "Estimate",
  subcateoryAlertHead: "Delete SubCategory",
  subcategoryAlertPara: "Are you sure you want to delete?",
  subcategoryDelete: "DELETE",
  subcategoryCancel: "CANCEL",
  subcategoryAddAlertMessages: "Sub Category added successfully",
  subcategoryDeleteAlertMessages: "Sub Category deleted successfully",
  subcategoryDeleteAlertErrorMessages: "Failed to delete sub category",

  //Sub Category List Screen
  subcategorylistmessage1: "No Items yet!",
  subcategorylistmessage2:
    "Add your expenses and prices to keep a mind of your transactions",
  subcategorylisttitle: "Add Item name and Price",
  subcategorylisttoggleinput1: "Item Name",
  subcategorylisttoggleinput2: "Price",
  subcategoryinputwarning: "Price should not be negative",
  subcateorylistAlertHead: "Delete Item",
  subcategorylistAlertPara: "Are you sure you want to delete?",
  subcategorylistDelete: "DELETE",
  subcategorylistCancel: "CANCEL",

  itemAddedSuccess: "Item added successfully",
  itemAddedError: "Failed to add item",
  itemDeleteSuccess: "Item deleted successfully",
  itemDeleteError: "Failed to delete Item",

  //Statistics Screen
  statisticstitle: "Spendings",
  statisticsmonthly: "Monthly Expenses",
  statisticsweekly: "Weekly Expeneses",
  statisticssoilmoisture: "Soil Moisture Analytics",
  selectYear: "Select year",
  selectMonth: "Select Month",
  selectPlaceholder: "Select",
  warning1: "No Chart yet!",
  warning2: " Add your expenses to get insights through graphs",

  //Profile Screen
  profiletitle: "Profile",
  profilename: "Name",
  profieemail: "Email",
  profilelogout: "Logout",
  profilepdftitle: " Export Data to PDF Form",
  profileprint: "Print",
  ProfileLogoutAlertHead: "Logout",
  ProfileLogoutAlertPara: "Are you sure you want to logout?",
  ProfileLogoutCancel: "CANCEL",
  ProfileLogoutLogout: "LOGOUT",

  //Soil Moisture screen
  soilmoisturetitle: "Soil Moisture Operations",
  soilmoisturebutton: "Click here to navigate",
  soilmoisturetitle1: "Soil Moisture",
  soilmoistureanalytics: "Soil Moisture Analytics Chart",
  clickonpump: "Click ON to start the water pump",
  on: "ON",
  off: "OFF",
  PumpOn: "Water Pump Turned ON successfully",
  PumpOff: "Water Pump turned OFF successfully",

  //Weather
  Pressure: "Pressure",
  Wind: "Wind",
  Humidity: "Humidity",

  //Horizontal Scroll
  categoryChart: "Category chart",
  reminderScroll: "Reminder",
  Scheme: "Scheme",

  //Scheme Section
  schemeTitle: "Goverment Schemes and services",

  //Scheme Flatlist
  //id1
  listTitle1: "Chief Minister Raitha Vidya Nidhi",
  description1:
    "Chief Minister Raitha Vidya Nidhi is a scheme in Karnataka, India that provides financial aid to the children of farmers pursuing higher education in agriculture and related fields. It offers scholarships of up to Rs. 38,000 per year to eligible students to cover their educational expenses.",

  //id2
  listTitle2: "Pradhan Mantri Kisan Samman Nidhi (PM KISAN)",
  description2:
    "Pradhan Mantri Kisan Samman Nidhi (PM KISAN) is a scheme by the Government of India that provides income support to small and marginal farmers. Eligible farmers receive Rs. 6,000 per year in three equal installments, directly into their bank accounts, to help with agricultural expenses and household needs.",

  //id3
  listTitle3: "Savayava Siri",
  description3:
    "Savayava Siri is a Karnataka government initiative to promote organic farming by providing subsidies, training, and support to farmers. Its aim is to encourage sustainable agriculture and protect the environment.",

  //id4
  listTitle4: "Rashtriya Krishi Vikas Yojana(RKVY RAFTAAR)",
  description4:
    "Rashtriya Krishi Vikas Yojana (RKVY RAFTAAR) is an Indian government scheme that promotes agricultural development by providing financial support to states for implementing innovative projects. It aims to achieve a 4% annual growth rate in the agriculture sector and focuses on enhancing productivity, value addition, and infrastructure.",

  //id5
  listTitle5: "Fertilizer and Manure",
  description5:
    "The Government of Karnataka offers a Fertilizer and Manure service to farmers, providing easy access to quality fertilizers and manure through subsidized distribution centers. The service also includes soil testing facilities to promote sustainable farming practices.",

  //id6
  listTitle6: "Krushi Bhagya",
  description6:
    "Krushi Bhagya is a Karnataka government scheme that provides financial support to small and marginal farmers for crop cultivation, including land preparation, seed treatment, and marketing. The scheme aims to improve agricultural productivity and is implemented by the Department of Agriculture, Government of Karnataka.",

  //id7
  listTitle7: "Seeds",
  description7:
    "The Government of Karnataka provides a Seeds service to promote the use of quality seeds for agricultural purposes. The service includes seed production and distribution centers, training, and technical assistance for farmers. It is administered by the Department of Agriculture, Government of Karnataka, to support the agricultural sector and promote sustainable farming practices.",

  //id8
  listTitle8: "Micro Irrigation",
  description8:
    "The Government of Karnataka provides Micro Irrigation services to promote efficient water use in agriculture. The service includes technical assistance, training, and financial support for adopting micro-irrigation technologies such as drip and sprinkler systems. The service is administered by the Department of Water Resources to promote sustainable water management practices in the state.",

  //id9
  listTitle9: "Farm Mechanization And Agro processing",
  description9:
    "The Government of Karnataka's Farm Mechanization and Agro-processing service provides financial assistance, training, and technical support to farmers for modern farming practices. This service helps increase agricultural productivity and income and is administered by the Department of Agriculture.",

  //id10
  listTitle10: "National Mission For Sustainable Agriculture(NMSA)",
  description10:
    "NMSA is a Government of India scheme promoting sustainable agriculture practices, including soil health, water conservation, and productivity improvement. It supports integrated farming, agroforestry, and farmer skill development, implemented by the Department of Agriculture.",

  //id11
  listTitle11: "Krushi Abhiyaana",
  description11:
    "Krushi Abhiyaana is a program by the Government of Karnataka to provide financial and technical support to farmers, promoting sustainable farming practices through soil health cards, training, and crop insurance. The program aims to increase crop yields, reduce input costs, and increase farmers' income, and is implemented by the Department of Agriculture.",

  //id12
  listTitle12: "National Food Security Mission(NFSM)",
  description12:
    "The National Food Security Mission (NFSM) increases food grain production and improves food security through better technology and farming practices for rice, wheat, and pulses. It's implemented by the Department of Agriculture.",

  //id13
  listTitle13: "ATMA",
  description13:
    "ATMA is a government program in India that promotes sustainable agricultural practices by providing training, extension services, and market linkages to farmers. It aims to improve productivity and income and is implemented by the Department of Agriculture.",

  //id14
  listTitle14: "TRAINING",
  description14:
    "Karnataka Government provides training to farmers on advanced farming techniques, pest management, and market linkages. The goal is to improve productivity and promote sustainable practices.",

  //id15
  listTitle15: "Plant Protection",
  description15:
    "The Plant Protection service of the Karnataka Government offers farmers information on pest control and management, conducts pest surveillance, and provides access to safe and effective pesticides. Its goal is to enhance agricultural productivity and ensure food security.",

  //id16
  listTitle16: "COMPENSATION FOR FARMERS SUICIDE",
  description16:
    "The Government of Karnataka offers compensation to the families of farmers who have committed suicide due to financial distress, including education assistance for their children. It is implemented by the Department of Agriculture, Karnataka.",

  //id17
  listTitle17: "State Pesticide Testing Laboratory",
  description17:
    "The State Pesticide Testing Laboratory of Karnataka Government tests pesticides to ensure safety and quality. It aims to ensure farmers have access to safe and effective pesticides.",

  //id18
  listTitle18: "Accidental Death and hayloss compensations",
  description18:
    "The Government of Karnataka offers compensation to farmers' families for accidental death or hay loss due to natural calamities to help them recover from the losses. It is implemented by the Department of Agriculture.",

  //id19
  listTitle19: "Discount for 1 percent Crop loan",
  description19:
    "The Karnataka government offers a 1% discount on crop loans up to Rs. 3 lakh taken by farmers from cooperative banks and other financial institutions. It aims to provide affordable credit to farmers for agricultural activities.",

  //id20
  listTitle20: "Karnataka Raitha Suraksha Pradhana Mantri Fasal Bima Yojana",
  description20:
    "The Karnataka crop insurance scheme, known as Raitha Suraksha Pradhana Mantri Fasal Bima Yojana, offers affordable crop insurance to farmers in case of crop loss or damage due to natural calamities, ensuring their financial stability.",

  //id21
  listTitle21: "Fertilizer and Biofertilizer Quality Control Laboratory",
  description21:
    "The Fertilizer and Biofertilizer Quality Control Lab in Karnataka tests fertilizers and biofertilizers for safety and effectiveness, issuing quality certificates to dealers and manufacturers",

  //id22
  listTitle22: "Raita Siri",
  description22:
    "Raita Siri is a government of Karnataka scheme that aims to provide financial assistance to small and marginal farmers in the state. Under this scheme, eligible farmers can receive Rs. 10,000 per hectare of land as direct benefit transfer (DBT) for a maximum of 10 hectares per farmer per year.",

  //id23
  listTitle23: "Raitha Samparka Kendra",
  description23:
    "Raitha Samparka Kendra - Karnataka government's initiative to establish centers for connecting farmers to agricultural experts, providing them with information and advice on farming practices, crop management, market trends, and other related issues.",

  //id24
  listTitle24: "Crop Survey Scheme",
  description24:
    "The Crop Survey Scheme is a Karnataka government initiative to collect crop data and develop strategies to enhance agricultural productivity and support farmers.",

  //id25
  listTitle25: "Maize D B T",
  description25:
    "Maize DBT is a government scheme that provides direct benefit transfer to farmers for the cultivation of maize crops.",

  //id26
  listTitle26: "Pesticides Licence Related",
  description26:
    "Karnataka Government provides Pesticide License to regulate the manufacture, sale, and distribution of pesticides in the state, ensuring the safety of farmers, consumers, and the environment.",

  //id27
  listTitle27: "Soil Health Mission State Sector",
  description27:
    "Karnataka's Soil Health Mission is a state-level initiative to promote soil health and sustainable agricultural practices. The program aims to increase crop productivity by improving soil fertility, nutrient management, and soil conservation through various interventions such as soil testing, balanced fertilizer use, and crop residue management.",

  //id28
  listTitle28: "Krishi Yantra Dhare(CHSC)",
  description28:
    "Krishi Yantra Dhare (CHSC) is a scheme by the Karnataka government aimed at providing agricultural machinery and equipment to farmers on a rental basis. The scheme aims to improve farm productivity by making modern and efficient farming equipment available to farmers at affordable prices.",

  readMoreEnglish: "Read more - English",
  readMoreKannada: "Read more - Kannada",
  closeScheme: "Close",
  tapToReadMore: "Tap to read More",

  //Reciept Management
  receiptButton: "Receipt Management",
  receiptContTitle: "Receipts",
  closeButton: "Close",

  //Reciept Management Form
  receiptButtonHead: "Click to add receipts",
  receiptCont2Title: "New Receipt",
  input1: "Receipt name",
  input2: "Add Image",
  input3: "Comment",
  submitReceipt: "Submit",

  //reciept management alert
  receiptAlertTitle: "Delete receipt",
  receiptAlertpara: "Are you sure you want to delete this receipt?",
  recieptSuccess: "New receipt added successfully",
  receiptDecline: "Error, receipt could not be created!",
  receiptCancel: "CANCEL",
  receiptDelete: "DELETE",

  receiptDeleteAlertSuccess: "Reminder deleted successfully",
  receiptDeleteAlertFailure: "Failed to delete reminder",
};

const kn = {
  //Login Screen
  welcome: "ಕೃಷಿ ಮಿತ್ರ",
  login: "ಲಾಗಿನ್",
  loginBtn: "ಸಲ್ಲಿಸು",
  loginInputPlaceholder: "ಇಮೇಲ್",
  passwordInputPlaceholder: "ಗುಪ್ತಪದ",
  signUpBtn: "ಸೈನ್ ಅಪ್ ಮಾಡಿ",
  or: "ಅಥವಾ",

  //Register Screen
  register: "ನೋಂದಣಿ",
  email: "ಇಮೇಲ್",
  fullname: "ಪೂರ್ಣ ಹೆಸರು",
  confirmpassword: "ಪಾಸ್ವರ್ಡ್ ದೃಢೀಕರಿಸಿ",
  message1: "ಕನಿಷ್ಠ ಪಾಸ್‌ವರ್ಡ್ ಉದ್ದವು 8 ಅಕ್ಷರಗಳಾಗಿರಬೇಕು!",
  message2: "ಪಾಸ್‌ವರ್ಡ್ ಹೊಂದಿಕೆಯಾಗುತ್ತಿಲ್ಲ!",
  RegisterAlert: "ಖಾತೆಯನ್ನು ರಚಿಸಲಾಗಿದೆ",
  RegisterError: "ನಿಮ್ಮ ಖಾತೆಯನ್ನು ರಚಿಸುವಲ್ಲಿ ದೋಷ!",

  //Home Screen
  hi: "ನಮಸ್ತೆ",
  totalCategories: "ಒಟ್ಟು ವರ್ಗಗಳು",
  reminder: "ಜ್ಞಾಪನೆ",

  //Category chart
  homeCategoryChart1: "ಇನ್ನೂ ಯಾವುದೇ ವರ್ಗಗಳಿಲ್ಲ!",
  homeCategoryChart2:
    "ನಿಮ್ಮ ವೆಚ್ಚಗಳೊಂದಿಗೆ ನ್ಯಾವಿಗೇಟ್ ಮಾಡಲು ನಿಮಗೆ ಸಹಾಯ ಮಾಡಲು ವರ್ಗವನ್ನು ಹೊಂದಿಸಿ",

  //Reminder Screen
  reminderMessage1: "ಯಾವುದೇ ಜ್ಞಾಪನೆಗಳನ್ನು ಹೊಂದಿಸಲಾಗಿಲ್ಲ!",
  reminderMessage2: "ನಂತರದ ವೆಚ್ಚಕ್ಕಾಗಿ ನಿಮಗೆ ನೆನಪಿಸಲು ಜ್ಞಾಪನೆಯನ್ನು ಸೇರಿಸಿ",
  reminderMessage3: "ಜ್ಞಾಪನೆಯನ್ನು ಸೇರಿಸಿ",
  Upcomingreminder: "ಮುಂಬರುವ ಜ್ಞಾಪನೆಗಳು",
  reminderMessage4: "ಜ್ಞಾಪನೆಗಳನ್ನು ಅಳಿಸಲು ದೀರ್ಘವಾಗಿ ಒತ್ತಿರಿ",
  reminderTitle: "ಜ್ಞಾಪನೆಯನ್ನು ಸೇರಿಸಿ",
  reminderDate: "ದಿನಾಂಕ",
  reminderTime: "ಸಮಯ",
  reminderInput: "ಟಿಪ್ಪಣಿಯನ್ನು ಸೇರಿಸಿ",
  reminderSubmit: "ಸಲ್ಲಿಸು",
  reminderRemaining: "ಉಳಿದಿರುವ ದಿನಗಳು",
  ReminderSucess: "ಜ್ಞಾಪನೆಯನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಹೊಂದಿಸಲಾಗಿದೆ",

  reminderDeleteSucess: "ಜ್ಞಾಪನೆಯನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಅಳಿಸಲಾಗಿದೆ",
  reminderAlertDeleteTitle: "ಜ್ಞಾಪನೆಯನ್ನು ಅಳಿಸಿ",
  reminderAlertDeleteMessage: "ಈ ಜ್ಞಾಪನೆಯನ್ನು ಅಳಿಸಲು ನೀವು ಖಚಿತವಾಗಿ ಬಯಸುವಿರಾ?",

  //Category Screen
  categorytitle: "ವರ್ಗಗಳು",
  categoryMessage1: "ಇನ್ನೂ ಯಾವುದೇ ವರ್ಗಗಳಿಲ್ಲ!",
  categoryMessage2:
    "ಹೊಸ ವರ್ಗವನ್ನು ಸೇರಿಸಲು ಮತ್ತು ಬಜೆಟ್ ಅನ್ನು ಸುಲಭಗೊಳಿಸಲು ಪ್ಲಸ್ ಮೇಲೆ ಕ್ಲಿಕ್ ಮಾಡಿ.",
  categorytoggle: "ವರ್ಗವನ್ನು ಸೇರಿಸಿ",
  caetgoryInput: "ವರ್ಗ",
  categorysubmit: "ಸಲ್ಲಿಸು",
  cateoryAlertHead: "ವರ್ಗವನ್ನು ಅಳಿಸಿ",
  categoryAlertPara: "ಈ ವರ್ಗವನ್ನು ಅಳಿಸಲು ನೀವು ಖಚಿತವಾಗಿ ಬಯಸುವಿರಾ?",
  categoryDelete: "ಅಳಿಸು",
  categoryCancel: "ರದ್ದುಮಾಡು",
  categoryAddAlertMessages: "ವರ್ಗವನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಸೇರಿಸಲಾಗಿದೆ",
  categoryAddAlertErrorMessages: "ವರ್ಗವನ್ನು ಸೇರಿಸಲು ವಿಫಲವಾಗಿದೆ",
  categoryDeleteAlertMessages: "ವರ್ಗವನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಅಳಿಸಲಾಗಿದೆ",
  categoryDeleteAlertErrorMessages: "ವರ್ಗವನ್ನು ಅಳಿಸಲು ವಿಫಲವಾಗಿದೆ",

  //Sub Category Screen
  subcategorytitle: "ಪಟ್ಟಿಗಳು",
  subcategorymessage1: "ಇನ್ನೂ ಯಾವುದೇ ಉಪ ವರ್ಗಗಳಿಲ್ಲ!",
  subcategorymessage2:
    "ನಿಮ್ಮ ಖರ್ಚುಗಳನ್ನು ಟ್ರ್ಯಾಕ್ ಮಾಡಲು ನಿಮಗೆ ಸಹಾಯ ಮಾಡಲು ಹೊಸ ಉಪ ವರ್ಗವನ್ನು ಹೊಂದಿಸಿ",
  subcategorytoggle: "ನಿಮ್ಮ ಪಟ್ಟಿಯನ್ನು ಸೇರಿಸಿ",
  subcategorytoggletitle: "ನಿಮ್ಮ ಪಟ್ಟಿಯನ್ನು ಹೆಸರಿಸಿ",
  subcategoryestimate: "ಅಂದಾಜು",

  //Sub Category List Screen
  subcategorylistmessage1: "ಇನ್ನೂ ಯಾವುದೇ ಐಟಂಗಳಿಲ್ಲ!",
  subcategorylistmessage2:
    "ನಿಮ್ಮ ವಹಿವಾಟುಗಳನ್ನು ಗಮನದಲ್ಲಿಟ್ಟುಕೊಳ್ಳಲು ನಿಮ್ಮ ವೆಚ್ಚಗಳು ಮತ್ತು ಬೆಲೆಗಳನ್ನು ಸೇರಿಸಿ",
  subcategorylisttitle: "ಐಟಂ ಹೆಸರು ಮತ್ತು ಬೆಲೆ ಸೇರಿಸಿ",
  subcategorylisttoggleinput1: "ವಸ್ತುವಿನ ಹೆಸರು",
  subcategorylisttoggleinput2: "ಬೆಲೆ",
  subcategoryinputwarning: "ಬೆಲೆ ಋಣಾತ್ಮಕವಾಗಿರಬಾರದು",
  subcateoryAlertHead: "ಉಪವರ್ಗವನ್ನು ಅಳಿಸಿ",
  subcategoryAlertPara: "ನೀವು ಅಳಿಸಲು ಖಚಿತವಾಗಿ ಬಯಸುವಿರಾ?",
  subcategoryDelete: "ಅಳಿಸು",
  subcategoryCancel: "ರದ್ದುಮಾಡು",

  subcategoryAddAlertMessages: "ಉಪ ವರ್ಗವನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಸೇರಿಸಲಾಗಿದೆ",
  subcategoryDeleteAlertMessages: "ಉಪವರ್ಗವನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಅಳಿಸಲಾಗಿದೆ",
  subcategoryDeleteAlertErrorMessages: "ಉಪ ವರ್ಗವನ್ನು ಅಳಿಸಲು ವಿಫಲವಾಗಿದೆ",

  subcateorylistAlertHead: "ಐಟಂ ಅಳಿಸಿ",
  subcategorylistAlertPara: "ನೀವು ಅಳಿಸಲು ಖಚಿತವಾಗಿ ಬಯಸುವಿರಾ?",
  subcategorylistDelete: "ಅಳಿಸು",
  subcategorylistCancel: "ರದ್ದುಮಾಡು",

  itemAddedSuccess: "ಐಟಂ ಅನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಸೇರಿಸಲಾಗಿದೆ",
  itemAddedError: "ಐಟಂ ಸೇರಿಸಲು ವಿಫಲವಾಗಿದೆ",
  itemDeleteSuccess: "ಐಟಂ ಅನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಅಳಿಸಲಾಗಿದೆ",
  itemDeleteError: "ಐಟಂ ಅಳಿಸಲು ವಿಫಲವಾಗಿದೆ",

  //Statistics Screen
  statisticstitle: "ಖರ್ಚುಗಳು",
  statisticsmonthly: "ಮಾಸಿಕ ವೆಚ್ಚಗಳು",
  statisticsweekly: "ಸಾಪ್ತಾಹಿಕ ವೆಚ್ಚಗಳು",
  statisticssoilmoisture: "ಮಣ್ಣಿನ ತೇವಾಂಶ ವಿಶ್ಲೇಷಣೆ",
  selectYear: "ವರ್ಷವನ್ನು ಆಯ್ಕೆಮಾಡಿ",
  selectMonth: "ತಿಂಗಳು ಆಯ್ಕೆಮಾಡಿ",
  selectPlaceholder: "ಆಯ್ಕೆ ಮಾಡಿ",
  warning1: "ಇನ್ನೂ ಚಾರ್ಟ್ ಇಲ್ಲ!",
  warning2: "ಗ್ರಾಫ್‌ಗಳ ಮೂಲಕ ಒಳನೋಟಗಳನ್ನು ಪಡೆಯಲು ನಿಮ್ಮ ವೆಚ್ಚಗಳನ್ನು ಸೇರಿಸಿ",

  //Profile Screen
  profiletitle: "ಪ್ರೊಫೈಲ್",
  profilename: "ಹೆಸರು",
  profieemail: "ಇಮೇಲ್",
  profilelogout: "ಲಾಗ್ ಔಟ್",
  profilepdftitle: "PDF ಫಾರ್ಮ್‌ಗೆ ಡೇಟಾವನ್ನು ರಫ್ತು ಮಾಡಿ",
  profileprint: "ಮುದ್ರಿಸಿ",
  ProfileLogoutAlertHead: "ಲಾಗ್ ಔಟ್",
  ProfileLogoutAlertPara: "ನೀವು ಲಾಗ್ಔಟ್ ಮಾಡಲು ಖಚಿತವಾಗಿ ಬಯಸುವಿರಾ?",
  ProfileLogoutCancel: "ರದ್ದುಮಾಡು",
  ProfileLogoutLogout: "ಲಾಗ್ ಔಟ್",

  //Soil Moisture screen
  soilmoisturetitle: "ಮಣ್ಣಿನ ತೇವಾಂಶ ಕಾರ್ಯಾಚರಣೆಗಳು",
  soilmoisturebutton: "ನ್ಯಾವಿಗೇಟ್ ಮಾಡಲು ಇಲ್ಲಿ ಕ್ಲಿಕ್ ಮಾಡಿ",
  soilmoisturetitle1: "ಮಣ್ಣಿನ ತೇವಾಂಶ",
  soilmoistureanalytics: "ಮಣ್ಣಿನ ತೇವಾಂಶ ವಿಶ್ಲೇಷಣೆ ಚಾರ್ಟ್",
  clickonpump: "ನೀರಿನ ಪಂಪ್ ಅನ್ನು ಪ್ರಾರಂಭಿಸಲು ಕ್ಲಿಕ್ ಮಾಡಿ",
  on: "ಆನ್",
  off: "ಆರಿಸಿ",
  PumpOn: "ನೀರಿನ ಪಂಪ್ ಯಶಸ್ವಿಯಾಗಿ ಆನ್ ಆಗಿದೆ",
  PumpOff: "ನೀರಿನ ಪಂಪ್ ಯಶಸ್ವಿಯಾಗಿ ಆಫ್ ಆಗಿದೆ",

  //Weather
  Pressure: "ಒತ್ತಡ",
  Wind: "ಗಾಳಿ",
  Humidity: "ಆರ್ದ್ರತೆ",

  //Horizontal Scroll
  categoryChart: "ವರ್ಗ ಚಾರ್ಟ್",
  reminderScroll: "ಜ್ಞಾಪನೆ",
  Scheme: "ಯೋಜನೆ",

  //Scheme Section
  schemeTitle: "ಸರ್ಕಾರದ ಯೋಜನೆಗಳು ಮತ್ತು ಸೇವೆಗಳು",

  //Scheme Flatlist
  //id1
  listTitle1: "ಮುಖ್ಯಮಂತ್ರಿ ರೈತ ವಿದ್ಯಾ ನಿಧಿ",
  description1:
    "ಮುಖ್ಯಮಂತ್ರಿ ರೈತ ವಿದ್ಯಾ ನಿಧಿಯು ಭಾರತದ ಕರ್ನಾಟಕದಲ್ಲಿ ಕೃಷಿ ಮತ್ತು ಸಂಬಂಧಿತ ಕ್ಷೇತ್ರಗಳಲ್ಲಿ ಉನ್ನತ ಶಿಕ್ಷಣವನ್ನು ಪಡೆಯುವ ರೈತರ ಮಕ್ಕಳಿಗೆ ಆರ್ಥಿಕ ನೆರವು ನೀಡುವ ಯೋಜನೆಯಾಗಿದೆ. ಇದು ರೂ.ವರೆಗಿನ ವಿದ್ಯಾರ್ಥಿವೇತನವನ್ನು ನೀಡುತ್ತದೆ. ತಮ್ಮ ಶೈಕ್ಷಣಿಕ ವೆಚ್ಚವನ್ನು ಭರಿಸಲು ಅರ್ಹ ವಿದ್ಯಾರ್ಥಿಗಳಿಗೆ ವರ್ಷಕ್ಕೆ 38,000.",

  //id2
  listTitle2: "ಪ್ರಧಾನ ಮಂತ್ರಿ ಕಿಸಾನ್ ಸಮ್ಮಾನ್ ನಿಧಿ (PM KISAN)",
  description2:
    "ಪ್ರಧಾನ ಮಂತ್ರಿ ಕಿಸಾನ್ ಸಮ್ಮಾನ್ ನಿಧಿ (PM KISAN) ಭಾರತ ಸರ್ಕಾರದ ಯೋಜನೆಯಾಗಿದ್ದು ಅದು ಸಣ್ಣ ಮತ್ತು ಅತಿ ಸಣ್ಣ ರೈತರಿಗೆ ಆದಾಯ ಬೆಂಬಲವನ್ನು ಒದಗಿಸುತ್ತದೆ. ಅರ್ಹ ರೈತರು ರೂ. ಕೃಷಿ ವೆಚ್ಚಗಳು ಮತ್ತು ಮನೆಯ ಅಗತ್ಯಗಳಿಗೆ ಸಹಾಯ ಮಾಡಲು ಅವರ ಬ್ಯಾಂಕ್ ಖಾತೆಗಳಿಗೆ ನೇರವಾಗಿ ಮೂರು ಸಮಾನ ಕಂತುಗಳಲ್ಲಿ ವರ್ಷಕ್ಕೆ 6,000 ರೂ.",

  //id3
  listTitle3: "ಸವಯವ ಸಿರಿ",
  description3:
    "ಸವಯವ ಸಿರಿಯು ರೈತರಿಗೆ ಸಹಾಯಧನ, ತರಬೇತಿ ಮತ್ತು ಬೆಂಬಲವನ್ನು ನೀಡುವ ಮೂಲಕ ಸಾವಯವ ಕೃಷಿಯನ್ನು ಉತ್ತೇಜಿಸಲು ಕರ್ನಾಟಕ ಸರ್ಕಾರದ ಉಪಕ್ರಮವಾಗಿದೆ. ಸುಸ್ಥಿರ ಕೃಷಿಯನ್ನು ಉತ್ತೇಜಿಸುವುದು ಮತ್ತು ಪರಿಸರವನ್ನು ರಕ್ಷಿಸುವುದು ಇದರ ಉದ್ದೇಶವಾಗಿದೆ.",

  //id4
  listTitle4: "ರಾಷ್ಟ್ರೀಯ ಕೃಷಿ ವಿಕಾಸ ಯೋಜನೆ (RKVY RAFTAAR)",
  description4:
    "ರಾಷ್ಟ್ರೀಯ ಕೃಷಿ ವಿಕಾಸ ಯೋಜನೆ (RKVY RAFTAAR) ಭಾರತ ಸರ್ಕಾರದ ಯೋಜನೆಯಾಗಿದ್ದು, ನವೀನ ಯೋಜನೆಗಳನ್ನು ಅನುಷ್ಠಾನಗೊಳಿಸಲು ರಾಜ್ಯಗಳಿಗೆ ಹಣಕಾಸಿನ ನೆರವು ನೀಡುವ ಮೂಲಕ ಕೃಷಿ ಅಭಿವೃದ್ಧಿಯನ್ನು ಉತ್ತೇಜಿಸುತ್ತದೆ. ಇದು ಕೃಷಿ ವಲಯದಲ್ಲಿ 4% ವಾರ್ಷಿಕ ಬೆಳವಣಿಗೆ ದರವನ್ನು ಸಾಧಿಸುವ ಗುರಿಯನ್ನು ಹೊಂದಿದೆ ಮತ್ತು ಉತ್ಪಾದಕತೆ, ಮೌಲ್ಯವರ್ಧನೆ ಮತ್ತು ಮೂಲಸೌಕರ್ಯವನ್ನು ಹೆಚ್ಚಿಸುವುದರ ಮೇಲೆ ಕೇಂದ್ರೀಕರಿಸುತ್ತದೆ.",

  //id5
  listTitle5: "ಗೊಬ್ಬರ ಮತ್ತು ಗೊಬ್ಬರ",
  description5:
    "ಕರ್ನಾಟಕ ಸರ್ಕಾರವು ರೈತರಿಗೆ ರಸಗೊಬ್ಬರ ಮತ್ತು ಗೊಬ್ಬರ ಸೇವೆಯನ್ನು ನೀಡುತ್ತದೆ, ಸಬ್ಸಿಡಿ ವಿತರಣಾ ಕೇಂದ್ರಗಳ ಮೂಲಕ ಗುಣಮಟ್ಟದ ರಸಗೊಬ್ಬರಗಳು ಮತ್ತು ಗೊಬ್ಬರವನ್ನು ಸುಲಭವಾಗಿ ಪ್ರವೇಶಿಸುತ್ತದೆ. ಸುಸ್ಥಿರ ಕೃಷಿ ಪದ್ಧತಿಗಳನ್ನು ಉತ್ತೇಜಿಸಲು ಈ ಸೇವೆಯು ಮಣ್ಣು ಪರೀಕ್ಷಾ ಸೌಲಭ್ಯಗಳನ್ನು ಸಹ ಒಳಗೊಂಡಿದೆ.",

  //id6
  listTitle6: "ಕ್ರುಶಿ ಭಾಗ್ಯ",
  description6:
    "ಕೃಷಿ ಭಾಗ್ಯವು ಕರ್ನಾಟಕ ಸರ್ಕಾರದ ಯೋಜನೆಯಾಗಿದ್ದು, ಸಣ್ಣ ಮತ್ತು ಅತಿ ಸಣ್ಣ ರೈತರಿಗೆ ಭೂಮಿ ತಯಾರಿಕೆ, ಬೀಜ ಸಂಸ್ಕರಣೆ ಮತ್ತು ಮಾರುಕಟ್ಟೆ ಸೇರಿದಂತೆ ಬೆಳೆ ಬೆಳೆಯಲು ಹಣಕಾಸಿನ ನೆರವು ನೀಡುತ್ತದೆ. ಈ ಯೋಜನೆಯು ಕೃಷಿ ಉತ್ಪಾದಕತೆಯನ್ನು ಸುಧಾರಿಸುವ ಗುರಿಯನ್ನು ಹೊಂದಿದೆ ಮತ್ತು ಇದನ್ನು ಕರ್ನಾಟಕ ಸರ್ಕಾರದ ಕೃಷಿ ಇಲಾಖೆಯು ಅನುಷ್ಠಾನಗೊಳಿಸುತ್ತದೆ.",

  //id7
  listTitle7: "ಬೀಜಗಳು",
  description7:
    "ಕೃಷಿ ಉದ್ದೇಶಗಳಿಗಾಗಿ ಗುಣಮಟ್ಟದ ಬೀಜಗಳ ಬಳಕೆಯನ್ನು ಉತ್ತೇಜಿಸಲು ಕರ್ನಾಟಕ ಸರ್ಕಾರವು ಬೀಜಗಳ ಸೇವೆಯನ್ನು ಒದಗಿಸುತ್ತದೆ. ಸೇವೆಯು ಬೀಜ ಉತ್ಪಾದನೆ ಮತ್ತು ವಿತರಣಾ ಕೇಂದ್ರಗಳು, ತರಬೇತಿ ಮತ್ತು ರೈತರಿಗೆ ತಾಂತ್ರಿಕ ಸಹಾಯವನ್ನು ಒಳಗೊಂಡಿದೆ. ಕೃಷಿ ಕ್ಷೇತ್ರವನ್ನು ಬೆಂಬಲಿಸಲು ಮತ್ತು ಸುಸ್ಥಿರ ಕೃಷಿ ಪದ್ಧತಿಗಳನ್ನು ಉತ್ತೇಜಿಸಲು ಇದು ಕರ್ನಾಟಕ ಸರ್ಕಾರದ ಕೃಷಿ ಇಲಾಖೆಯಿಂದ ನಿರ್ವಹಿಸಲ್ಪಡುತ್ತದೆ.",

  //id8
  listTitle8: "ಮೈಕ್ರೋ ಇರಿಗೇಷನ್",
  description8:
    "ಕೃಷಿಯಲ್ಲಿ ಸಮರ್ಥ ನೀರಿನ ಬಳಕೆಯನ್ನು ಉತ್ತೇಜಿಸಲು ಕರ್ನಾಟಕ ಸರ್ಕಾರವು ಸೂಕ್ಷ್ಮ ನೀರಾವರಿ ಸೇವೆಗಳನ್ನು ಒದಗಿಸುತ್ತದೆ. ಈ ಸೇವೆಯು ತಾಂತ್ರಿಕ ನೆರವು, ತರಬೇತಿ ಮತ್ತು ಹನಿ ಮತ್ತು ಸ್ಪ್ರಿಂಕ್ಲರ್ ವ್ಯವಸ್ಥೆಗಳಂತಹ ಸೂಕ್ಷ್ಮ ನೀರಾವರಿ ತಂತ್ರಜ್ಞಾನಗಳನ್ನು ಅಳವಡಿಸಿಕೊಳ್ಳಲು ಹಣಕಾಸಿನ ಬೆಂಬಲವನ್ನು ಒಳಗೊಂಡಿದೆ. ರಾಜ್ಯದಲ್ಲಿ ಸುಸ್ಥಿರ ನೀರು ನಿರ್ವಹಣಾ ಪದ್ಧತಿಗಳನ್ನು ಉತ್ತೇಜಿಸಲು ಜಲಸಂಪನ್ಮೂಲ ಇಲಾಖೆಯು ಈ ಸೇವೆಯನ್ನು ನಿರ್ವಹಿಸುತ್ತದೆ.",

  //id9
  listTitle9: "ಫಾರ್ಮ್ ಯಾಂತ್ರೀಕರಣ ಮತ್ತು ಕೃಷಿ ಸಂಸ್ಕರಣೆ",
  description9:
    "ಕರ್ನಾಟಕ ಸರ್ಕಾರದ ಫಾರ್ಮ್ ಯಾಂತ್ರೀಕರಣ ಮತ್ತು ಕೃಷಿ ಸಂಸ್ಕರಣಾ ಸೇವೆಯು ಆಧುನಿಕ ಕೃಷಿ ಪದ್ಧತಿಗಳಿಗಾಗಿ ರೈತರಿಗೆ ಹಣಕಾಸಿನ ನೆರವು, ತರಬೇತಿ ಮತ್ತು ತಾಂತ್ರಿಕ ಬೆಂಬಲವನ್ನು ಒದಗಿಸುತ್ತದೆ. ಈ ಸೇವೆಯು ಕೃಷಿ ಉತ್ಪಾದಕತೆ ಮತ್ತು ಆದಾಯವನ್ನು ಹೆಚ್ಚಿಸಲು ಸಹಾಯ ಮಾಡುತ್ತದೆ ಮತ್ತು ಕೃಷಿ ಇಲಾಖೆಯಿಂದ ನಿರ್ವಹಿಸಲ್ಪಡುತ್ತದೆ.",

  //id10
  listTitle10: "ಸುಸ್ಥಿರ ಕೃಷಿಗಾಗಿ ರಾಷ್ಟ್ರೀಯ ಮಿಷನ್ (NMSA)",
  description10:
    "NMSA ಭಾರತ ಸರ್ಕಾರದ ಯೋಜನೆಯಾಗಿದ್ದು, ಮಣ್ಣಿನ ಆರೋಗ್ಯ, ನೀರಿನ ಸಂರಕ್ಷಣೆ ಮತ್ತು ಉತ್ಪಾದಕತೆ ಸುಧಾರಣೆ ಸೇರಿದಂತೆ ಸುಸ್ಥಿರ ಕೃಷಿ ಪದ್ಧತಿಗಳನ್ನು ಉತ್ತೇಜಿಸುತ್ತದೆ. ಇದು ಕೃಷಿ ಇಲಾಖೆಯಿಂದ ಜಾರಿಗೊಳಿಸಲಾದ ಸಮಗ್ರ ಕೃಷಿ, ಕೃಷಿ ಅರಣ್ಯ ಮತ್ತು ರೈತರ ಕೌಶಲ್ಯ ಅಭಿವೃದ್ಧಿಯನ್ನು ಬೆಂಬಲಿಸುತ್ತದೆ.",

  //id11
  listTitle11: "ಕ್ರುಶಿ ಅಭಿಯಾನ",
  description11:
    "ಕೃಷಿ ಅಭಿಯಾನವು ಕರ್ನಾಟಕ ಸರ್ಕಾರವು ರೈತರಿಗೆ ಆರ್ಥಿಕ ಮತ್ತು ತಾಂತ್ರಿಕ ಬೆಂಬಲವನ್ನು ಒದಗಿಸಲು, ಮಣ್ಣಿನ ಆರೋಗ್ಯ ಕಾರ್ಡ್‌ಗಳು, ತರಬೇತಿ ಮತ್ತು ಬೆಳೆ ವಿಮೆಯ ಮೂಲಕ ಸುಸ್ಥಿರ ಕೃಷಿ ಪದ್ಧತಿಗಳನ್ನು ಉತ್ತೇಜಿಸುವ ಕಾರ್ಯಕ್ರಮವಾಗಿದೆ. ಈ ಕಾರ್ಯಕ್ರಮವು ಬೆಳೆಗಳ ಇಳುವರಿಯನ್ನು ಹೆಚ್ಚಿಸಲು, ಇನ್ಪುಟ್ ವೆಚ್ಚವನ್ನು ಕಡಿಮೆ ಮಾಡಲು ಮತ್ತು ರೈತರ ಆದಾಯವನ್ನು ಹೆಚ್ಚಿಸಲು ಗುರಿಯನ್ನು ಹೊಂದಿದೆ ಮತ್ತು ಇದನ್ನು ಕೃಷಿ ಇಲಾಖೆಯು ಅನುಷ್ಠಾನಗೊಳಿಸುತ್ತದೆ.",

  //id12
  listTitle12: "ರಾಷ್ಟ್ರೀಯ ಆಹಾರ ಭದ್ರತಾ ಮಿಷನ್ (NFSM)",
  description12:
    "ರಾಷ್ಟ್ರೀಯ ಆಹಾರ ಭದ್ರತಾ ಮಿಷನ್ (NFSM) ಆಹಾರ ಧಾನ್ಯ ಉತ್ಪಾದನೆಯನ್ನು ಹೆಚ್ಚಿಸುತ್ತದೆ ಮತ್ತು ಅಕ್ಕಿ, ಗೋಧಿ ಮತ್ತು ಬೇಳೆಕಾಳುಗಳಿಗೆ ಉತ್ತಮ ತಂತ್ರಜ್ಞಾನ ಮತ್ತು ಕೃಷಿ ಪದ್ಧತಿಗಳ ಮೂಲಕ ಆಹಾರ ಭದ್ರತೆಯನ್ನು ಸುಧಾರಿಸುತ್ತದೆ. ಇದನ್ನು ಕೃಷಿ ಇಲಾಖೆ ಅನುಷ್ಠಾನಗೊಳಿಸಿದೆ.",

  //id13
  listTitle13: "ಎಟಿಎಂಎ",
  description13:
    "ATMA ಎಂಬುದು ಭಾರತದಲ್ಲಿನ ಸರ್ಕಾರಿ ಕಾರ್ಯಕ್ರಮವಾಗಿದ್ದು, ರೈತರಿಗೆ ತರಬೇತಿ, ವಿಸ್ತರಣಾ ಸೇವೆಗಳು ಮತ್ತು ಮಾರುಕಟ್ಟೆ ಸಂಪರ್ಕಗಳನ್ನು ಒದಗಿಸುವ ಮೂಲಕ ಸುಸ್ಥಿರ ಕೃಷಿ ಪದ್ಧತಿಗಳನ್ನು ಉತ್ತೇಜಿಸುತ್ತದೆ. ಇದು ಉತ್ಪಾದಕತೆ ಮತ್ತು ಆದಾಯವನ್ನು ಸುಧಾರಿಸುವ ಗುರಿಯನ್ನು ಹೊಂದಿದೆ ಮತ್ತು ಇದನ್ನು ಕೃಷಿ ಇಲಾಖೆಯು ಅನುಷ್ಠಾನಗೊಳಿಸುತ್ತದೆ.",

  //id14
  listTitle14: "ತರಬೇತಿ",
  description14:
    "ಕರ್ನಾಟಕ ಸರ್ಕಾರವು ರೈತರಿಗೆ ಸುಧಾರಿತ ಕೃಷಿ ತಂತ್ರಗಳು, ಕೀಟ ನಿರ್ವಹಣೆ ಮತ್ತು ಮಾರುಕಟ್ಟೆ ಸಂಪರ್ಕಗಳ ಕುರಿತು ತರಬೇತಿಯನ್ನು ನೀಡುತ್ತದೆ. ಉತ್ಪಾದಕತೆಯನ್ನು ಸುಧಾರಿಸುವುದು ಮತ್ತು ಸುಸ್ಥಿರ ಅಭ್ಯಾಸಗಳನ್ನು ಉತ್ತೇಜಿಸುವುದು ಗುರಿಯಾಗಿದೆ.",

  //id15
  listTitle15: "ಸಸ್ಯ ರಕ್ಷಣೆ",
  description15:
    "ಕರ್ನಾಟಕ ಸರ್ಕಾರದ ಸಸ್ಯ ಸಂರಕ್ಷಣಾ ಸೇವೆಯು ರೈತರಿಗೆ ಕೀಟ ನಿಯಂತ್ರಣ ಮತ್ತು ನಿರ್ವಹಣೆಯ ಮಾಹಿತಿಯನ್ನು ನೀಡುತ್ತದೆ, ಕೀಟ ಕಣ್ಗಾವಲು ನಡೆಸುತ್ತದೆ ಮತ್ತು ಸುರಕ್ಷಿತ ಮತ್ತು ಪರಿಣಾಮಕಾರಿ ಕೀಟನಾಶಕಗಳಿಗೆ ಪ್ರವೇಶವನ್ನು ಒದಗಿಸುತ್ತದೆ. ಕೃಷಿ ಉತ್ಪಾದಕತೆಯನ್ನು ಹೆಚ್ಚಿಸುವುದು ಮತ್ತು ಆಹಾರ ಭದ್ರತೆಯನ್ನು ಖಚಿತಪಡಿಸುವುದು ಇದರ ಗುರಿಯಾಗಿದೆ.",

  //id16
  listTitle16: "ರೈತರ ಆತ್ಮಹತ್ಯೆಗೆ ಪರಿಹಾರ",
  description16:
    "ಕರ್ನಾಟಕ ಸರ್ಕಾರವು ಆರ್ಥಿಕ ಸಂಕಷ್ಟದಿಂದ ಆತ್ಮಹತ್ಯೆ ಮಾಡಿಕೊಂಡ ರೈತರ ಕುಟುಂಬಗಳಿಗೆ ಅವರ ಮಕ್ಕಳಿಗೆ ಶಿಕ್ಷಣದ ನೆರವು ಸೇರಿದಂತೆ ಪರಿಹಾರವನ್ನು ನೀಡುತ್ತದೆ. ಇದನ್ನು ಕರ್ನಾಟಕ ಕೃಷಿ ಇಲಾಖೆ ಅನುಷ್ಠಾನಗೊಳಿಸಿದೆ.",

  //id17
  listTitle17: "ರಾಜ್ಯ ಕೀಟನಾಶಕ ಪರೀಕ್ಷಾ ಪ್ರಯೋಗಾಲಯ",
  description17:
    "ಕರ್ನಾಟಕ ಸರ್ಕಾರದ ರಾಜ್ಯ ಕೀಟನಾಶಕ ಪರೀಕ್ಷಾ ಪ್ರಯೋಗಾಲಯವು ಸುರಕ್ಷತೆ ಮತ್ತು ಗುಣಮಟ್ಟವನ್ನು ಖಚಿತಪಡಿಸಿಕೊಳ್ಳಲು ಕೀಟನಾಶಕಗಳನ್ನು ಪರೀಕ್ಷಿಸುತ್ತದೆ. ರೈತರಿಗೆ ಸುರಕ್ಷಿತ ಮತ್ತು ಪರಿಣಾಮಕಾರಿ ಕೀಟನಾಶಕಗಳ ಪ್ರವೇಶವನ್ನು ಖಚಿತಪಡಿಸುವುದು ಇದರ ಗುರಿಯಾಗಿದೆ.",

  //id18
  listTitle18: "ಆಕಸ್ಮಿಕ ಸಾವು ಮತ್ತು ಹೇಲೋಸ್ ಪರಿಹಾರಗಳು",
  description18:
    "ಕರ್ನಾಟಕ ಸರ್ಕಾರವು ರೈತರ ಕುಟುಂಬಗಳಿಗೆ ಆಕಸ್ಮಿಕ ಸಾವು ಅಥವಾ ನೈಸರ್ಗಿಕ ವಿಕೋಪಗಳಿಂದ ಹುಲ್ಲು ನಷ್ಟಕ್ಕೆ ಪರಿಹಾರವನ್ನು ನೀಡುತ್ತದೆ, ನಷ್ಟದಿಂದ ಚೇತರಿಸಿಕೊಳ್ಳಲು ಸಹಾಯ ಮಾಡುತ್ತದೆ. ಇದನ್ನು ಕೃಷಿ ಇಲಾಖೆ ಅನುಷ್ಠಾನಗೊಳಿಸಿದೆ.",

  //id19
  listTitle19: "1 ರಷ್ಟು ಬೆಳೆ ಸಾಲಕ್ಕೆ ರಿಯಾಯಿತಿ",
  description19:
    "ಕರ್ನಾಟಕ ಸರ್ಕಾರವು ರೂ.ವರೆಗಿನ ಬೆಳೆ ಸಾಲದ ಮೇಲೆ 1% ರಿಯಾಯಿತಿಯನ್ನು ನೀಡುತ್ತದೆ. ಸಹಕಾರಿ ಬ್ಯಾಂಕ್ ಮತ್ತು ಇತರೆ ಹಣಕಾಸು ಸಂಸ್ಥೆಗಳಿಂದ ರೈತರು ತೆಗೆದುಕೊಂಡ 3 ಲಕ್ಷ ರೂ. ಇದು ಕೃಷಿ ಚಟುವಟಿಕೆಗಳಿಗೆ ರೈತರಿಗೆ ಕೈಗೆಟುಕುವ ಸಾಲವನ್ನು ಒದಗಿಸುವ ಗುರಿಯನ್ನು ಹೊಂದಿದೆ.",

  //id20
  listTitle20: "ಕರ್ನಾಟಕ ರೈತ ಸುರಕ್ಷಾ ಪ್ರಧಾನ ಮಂತ್ರಿ ಫಸಲ್ ಬಿಮಾ ಯೋಜನೆ",
  description20:
    "ರೈತ ಸುರಕ್ಷಾ ಪ್ರಧಾನ ಮಂತ್ರಿ ಫಸಲ್ ಬಿಮಾ ಯೋಜನೆ ಎಂದು ಕರೆಯಲ್ಪಡುವ ಕರ್ನಾಟಕ ಬೆಳೆ ವಿಮಾ ಯೋಜನೆಯು ರೈತರಿಗೆ ಬೆಳೆ ನಷ್ಟ ಅಥವಾ ನೈಸರ್ಗಿಕ ವಿಕೋಪಗಳಿಂದ ಹಾನಿಯ ಸಂದರ್ಭದಲ್ಲಿ ಕೈಗೆಟುಕುವ ಬೆಳೆ ವಿಮೆಯನ್ನು ನೀಡುತ್ತದೆ, ಅವರ ಆರ್ಥಿಕ ಸ್ಥಿರತೆಯನ್ನು ಖಚಿತಪಡಿಸುತ್ತದೆ.",

  //id21
  listTitle21: "ರಸಗೊಬ್ಬರ ಮತ್ತು ಜೈವಿಕ ಗೊಬ್ಬರ ಗುಣಮಟ್ಟ ನಿಯಂತ್ರಣ ಪ್ರಯೋಗಾಲಯ",
  description21:
    "ಕರ್ನಾಟಕದಲ್ಲಿನ ರಸಗೊಬ್ಬರ ಮತ್ತು ಜೈವಿಕ ಗೊಬ್ಬರ ಗುಣಮಟ್ಟ ನಿಯಂತ್ರಣ ಪ್ರಯೋಗಾಲಯವು ರಸಗೊಬ್ಬರಗಳು ಮತ್ತು ಜೈವಿಕ ಗೊಬ್ಬರಗಳನ್ನು ಸುರಕ್ಷತೆ ಮತ್ತು ಪರಿಣಾಮಕಾರಿತ್ವಕ್ಕಾಗಿ ಪರೀಕ್ಷಿಸುತ್ತದೆ, ವಿತರಕರು ಮತ್ತು ತಯಾರಕರಿಗೆ ಗುಣಮಟ್ಟದ ಪ್ರಮಾಣಪತ್ರಗಳನ್ನು ನೀಡುತ್ತದೆ.",

  //id22
  listTitle22: "ರೈತ ಸಿರಿ",
  description22:
    "ರೈತ ಸಿರಿ ಕರ್ನಾಟಕ ಸರ್ಕಾರದ ಯೋಜನೆಯಾಗಿದ್ದು, ಇದು ರಾಜ್ಯದ ಸಣ್ಣ ಮತ್ತು ಅತಿ ಸಣ್ಣ ರೈತರಿಗೆ ಆರ್ಥಿಕ ನೆರವು ನೀಡುವ ಗುರಿಯನ್ನು ಹೊಂದಿದೆ. ಈ ಯೋಜನೆಯಡಿ ಅರ್ಹ ರೈತರು ರೂ. ಪ್ರತಿ ಹೆಕ್ಟೇರ್ ಭೂಮಿಗೆ 10,000 ನೇರ ಲಾಭ ವರ್ಗಾವಣೆಯಾಗಿ (DBT) ಪ್ರತಿ ರೈತನಿಗೆ ವರ್ಷಕ್ಕೆ ಗರಿಷ್ಠ 10 ಹೆಕ್ಟೇರ್.",

  //id23
  listTitle23: "ರೈತ ಸಂಪರ್ಕ ಕೇಂದ್ರ",
  description23:
    "ರೈತ ಸಂಪರ್ಕ ಕೇಂದ್ರ - ಕರ್ನಾಟಕ ಸರ್ಕಾರದ ಉಪಕ್ರಮವು ರೈತರನ್ನು ಕೃಷಿ ತಜ್ಞರೊಂದಿಗೆ ಸಂಪರ್ಕಿಸಲು ಕೇಂದ್ರಗಳನ್ನು ಸ್ಥಾಪಿಸಲು, ಅವರಿಗೆ ಕೃಷಿ ಪದ್ಧತಿಗಳು, ಬೆಳೆ ನಿರ್ವಹಣೆ, ಮಾರುಕಟ್ಟೆ ಪ್ರವೃತ್ತಿಗಳು ಮತ್ತು ಇತರ ಸಂಬಂಧಿತ ವಿಷಯಗಳ ಬಗ್ಗೆ ಮಾಹಿತಿ ಮತ್ತು ಸಲಹೆಗಳನ್ನು ನೀಡುತ್ತದೆ.",

  //id24
  listTitle24: "ಬೆಳೆ ಸಮೀಕ್ಷೆ ಯೋಜನೆ",
  description24:
    "ಬೆಳೆ ಸಮೀಕ್ಷೆ ಯೋಜನೆಯು ಬೆಳೆ ಡೇಟಾವನ್ನು ಸಂಗ್ರಹಿಸಲು ಮತ್ತು ಕೃಷಿ ಉತ್ಪಾದಕತೆಯನ್ನು ಹೆಚ್ಚಿಸಲು ಮತ್ತು ರೈತರನ್ನು ಬೆಂಬಲಿಸಲು ತಂತ್ರಗಳನ್ನು ಅಭಿವೃದ್ಧಿಪಡಿಸಲು ಕರ್ನಾಟಕ ಸರ್ಕಾರದ ಉಪಕ್ರಮವಾಗಿದೆ.",

  //id25
  listTitle25: "ಮೆಕ್ಕೆಜೋಳ ಡಿ ಬಿ ಟಿ",
  description25:
    "ಮೆಕ್ಕೆ ಜೋಳದ ಡಿಬಿಟಿಯು ಮೆಕ್ಕೆಜೋಳದ ಬೆಳೆಗಳ ಕೃಷಿಗಾಗಿ ರೈತರಿಗೆ ನೇರ ಲಾಭ ವರ್ಗಾವಣೆಯನ್ನು ಒದಗಿಸುವ ಸರ್ಕಾರಿ ಯೋಜನೆಯಾಗಿದೆ.",

  //id26
  listTitle26: "ಕೀಟನಾಶಕಗಳ ಪರವಾನಗಿ ಸಂಬಂಧಿತ",
  description26:
    "ಕರ್ನಾಟಕ ಸರ್ಕಾರವು ರೈತರು, ಗ್ರಾಹಕರು ಮತ್ತು ಪರಿಸರದ ಸುರಕ್ಷತೆಯನ್ನು ಖಾತ್ರಿಪಡಿಸುವ ಮೂಲಕ ರಾಜ್ಯದಲ್ಲಿ ಕೀಟನಾಶಕಗಳ ತಯಾರಿಕೆ, ಮಾರಾಟ ಮತ್ತು ವಿತರಣೆಯನ್ನು ನಿಯಂತ್ರಿಸಲು ಕೀಟನಾಶಕ ಪರವಾನಗಿಯನ್ನು ಒದಗಿಸುತ್ತದೆ.",

  //id27
  listTitle27: "ಮಣ್ಣಿನ ಆರೋಗ್ಯ ಮಿಷನ್ ರಾಜ್ಯ ವಲಯ",
  description27:
    "ಕರ್ನಾಟಕದ ಮಣ್ಣಿನ ಆರೋಗ್ಯ ಮಿಷನ್ ಮಣ್ಣಿನ ಆರೋಗ್ಯ ಮತ್ತು ಸುಸ್ಥಿರ ಕೃಷಿ ಪದ್ಧತಿಗಳನ್ನು ಉತ್ತೇಜಿಸಲು ರಾಜ್ಯ ಮಟ್ಟದ ಉಪಕ್ರಮವಾಗಿದೆ. ಮಣ್ಣಿನ ಪರೀಕ್ಷೆ, ಸಮತೋಲಿತ ರಸಗೊಬ್ಬರ ಬಳಕೆ ಮತ್ತು ಬೆಳೆ ಶೇಷ ನಿರ್ವಹಣೆಯಂತಹ ವಿವಿಧ ಮಧ್ಯಸ್ಥಿಕೆಗಳ ಮೂಲಕ ಮಣ್ಣಿನ ಫಲವತ್ತತೆ, ಪೋಷಕಾಂಶಗಳ ನಿರ್ವಹಣೆ ಮತ್ತು ಮಣ್ಣಿನ ಸಂರಕ್ಷಣೆಯನ್ನು ಸುಧಾರಿಸುವ ಮೂಲಕ ಬೆಳೆ ಉತ್ಪಾದಕತೆಯನ್ನು ಹೆಚ್ಚಿಸುವ ಗುರಿಯನ್ನು ಈ ಕಾರ್ಯಕ್ರಮ ಹೊಂದಿದೆ.",

  //id28
  listTitle28: "ಕೃಷಿ ಯಂತ್ರ ಧಾರೆ(CHSC)",
  description28:
    "ಕೃಷಿ ಯಂತ್ರ ಧಾರೆ (CHSC) ಕರ್ನಾಟಕ ಸರ್ಕಾರದ ಯೋಜನೆಯಾಗಿದ್ದು, ರೈತರಿಗೆ ಬಾಡಿಗೆ ಆಧಾರದ ಮೇಲೆ ಕೃಷಿ ಯಂತ್ರೋಪಕರಣಗಳು ಮತ್ತು ಸಲಕರಣೆಗಳನ್ನು ಒದಗಿಸುವ ಗುರಿಯನ್ನು ಹೊಂದಿದೆ. ಆಧುನಿಕ ಮತ್ತು ಪರಿಣಾಮಕಾರಿ ಕೃಷಿ ಉಪಕರಣಗಳನ್ನು ಕೈಗೆಟುಕುವ ಬೆಲೆಯಲ್ಲಿ ರೈತರಿಗೆ ಲಭ್ಯವಾಗುವಂತೆ ಮಾಡುವ ಮೂಲಕ ಕೃಷಿ ಉತ್ಪಾದಕತೆಯನ್ನು ಸುಧಾರಿಸುವ ಗುರಿಯನ್ನು ಈ ಯೋಜನೆ ಹೊಂದಿದೆ.",

  readMoreEnglish: "ಹೆಚ್ಚು ಓದಿ - ಇಂಗ್ಲೀಷ್",
  readMoreKannada: "ಮುಂದೆ ಓದಿ - ಕನ್ನಡ",
  closeScheme: "ಮುಚ್ಚಿ",
  tapToReadMore: "ಇನ್ನಷ್ಟು ಓದಲು ಟ್ಯಾಪ್ ಮಾಡಿ",

  //Reciept Management
  receiptButtonHead: "ರಸೀದಿಗಳನ್ನು ಸೇರಿಸಲು ಕ್ಲಿಕ್ ಮಾಡಿ",
  receiptButton: "ರಶೀದಿ ನಿರ್ವಹಣೆ",
  receiptContTitle: "ರಸೀದಿಗಳು",
  closeButton: "ಮುಚ್ಚಿ",
  //Reciept Management Form
  receiptCont2Title: "ಹೊಸ ರಸೀದಿ",
  input1: "ರಶೀದಿ ಹೆಸರು",
  input2: "ಚಿತ್ರವನ್ನು ಸೇರಿಸಿ",
  input3: "ಕಾಮೆಂಟ್ ಮಾಡಿ",
  submitReceipt: "ಸಲ್ಲಿಸು",
  //reciept management alert
  receiptAlertTitle: "ರಶೀದಿಯನ್ನು ಅಳಿಸಿ",
  receiptAlertpara: "ಈ ರಸೀದಿಯನ್ನು ಅಳಿಸಲು ನೀವು ಖಚಿತವಾಗಿ ಬಯಸುವಿರಾ?",
  recieptSuccess: "ಹೊಸ ರಸೀದಿಯನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಸೇರಿಸಲಾಗಿದೆ",
  receiptDecline: "ದೋಷ, ರಶೀದಿಯನ್ನು ರಚಿಸಲಾಗಲಿಲ್ಲ!",
  receiptCancel: "ರದ್ದುಮಾಡು",
  receiptDelete: "ಅಳಿಸು",

  receiptDeleteAlertSuccess: "ಜ್ಞಾಪನೆಯನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಅಳಿಸಲಾಗಿದೆ",
  receiptDeleteAlertFailure: "ಜ್ಞಾಪನೆಯನ್ನು ಅಳಿಸಲು ವಿಫಲವಾಗಿದೆ",
};

export { en, kn };
